...
export default function InstituteSettingsPage() {
    ...
    const fetchInstituteData = async () => {
        try {
            const response = await fetch(
                "http://raghuvatsal-001-site1.anytempurl.com/api/Institute/DISP_M_INSTITUTE"
            )
            ...
            // ❗ Handle no data case
            if (result.msgId !== 1 || !result.data) {
                console.warn("No institute data found")
                return
            }

            const institute = Array.isArray(result.data)
                ? result.data[0]
                : result.data

            // ✅ Fill form
            Object.keys(institute).forEach((key) => {
                setValue(key as keyof InstituteFormValues, institute[key])
            })
    ...
    const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                setValue("logO_URL", result) // Or handle file upload to server first
            }
    ...
    async function onSubmit(data: InstituteFormValues) {
        ...
        try {
            ...
            // Re-fetch data to keep the form sync and autofilled correctly
                ...
                <Card className="overflow-hidden border-none shadow-xl shadow-muted/50 transition-all hover:shadow-2xl hover:shadow-muted/60">
                    ...
                    <CardContent className="grid gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <Field>
                            ...
                            <Input {...register("institutE_CODE")} placeholder="ABC-000" />
                            ...
                            <Field>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        <div className="relative flex h-24 w-24 shrink-0 items-center justify-center rounded-2xl border-2 border-dashed border-muted-foreground/20 bg-muted/30 overflow-hidden group">
                                            {logoPreview ? (
                                                <>
                                        ...
                                        <div className="grid gap-2">
                                            <div className="flex gap-2">
                                                <label className="cursor-pointer">
                                                    <div className="flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary rounded-xl text-sm font-semibold hover:bg-primary/20 transition-colors">
                                            ...
                                            <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold">Max size 2MB (PNG, JPG)</p>
                                        </div>
                                    </div>
                                </div>
                            </Field>
